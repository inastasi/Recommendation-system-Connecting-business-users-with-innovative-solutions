...
class DataPreprocessing:
    def __init__(self, prj_data_file, load_trans_file, save_trans_path, save_path, 
                 custom_stop_words_flag=False, custom_stop_words_file=""):
        
        
        self.input_prj = pd.read_csv(prj_data_file, engine ='python')
        self.load_trans_file = load_trans_file # File which contains translated descriptions
    ...
    def split_spec_char(word_list, ch):
    ...
    def read_stop_words(self):
        ...
        '''Read csv file which contains custom stop words.'''
    ...
    def process(self):
        ...
        lang_model = fasttext.load_model('D:\\Various documents\\Masters\\Subjects\\Thesis\\Code\\lid.176.ftz')
        ...
        if self.load_trans_file != "":
            ...
            input_prj_trans = input_prj_trans.drop(input_prj_trans.columns.difference(['Description_text_en','Solution: Solution Name']), 1, inplace=True)
            print(input_prj_trans.columns)
            
            # Rename column Description_text_en to Description
            input_prj_trans.rename(columns={'Description_text_en':'Description'})
            
            # Merge project originaly in english with translated ones  
            clean_prj = pd.concat([clean_prj,input_prj_trans], ignore_index=True)
        
        # Remove URLs
        clean_prj['Description'] = clean_prj['Description'].apply(self.remove_url)
        ...
        clean_prj['nltk_tokens_pos'] = clean_prj['nltk_tokens_pos'].apply(lambda x: [el for el in x if regex.search(el[0]) == None])
        
        #Remove empty tokens
        clean_prj['nltk_tokens_pos'] = clean_prj['nltk_tokens_pos'].apply(lambda x: [el for el in x if el[0]!=''])
        
        # Remove stop words
        stop = stopwords.words('english')
        clean_prj['nltk_tokens_pos'] = clean_prj['nltk_tokens_pos'].apply(lambda x: [el for el in x if el[0] not in stop])
        ...
        # Save cleaned text
        clean_prj['Solution: Solution Name','Description'].to_csv(self.save_path+'Projects_desc_cleaned.csv', ignore_index=True)
